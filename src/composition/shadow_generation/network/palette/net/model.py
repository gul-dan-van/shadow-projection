        ...
        for train_data in tqdm.tqdm(self.phase_loader):
            ...
            if self.iter % self.opt['train']['log_iter'] == 0:
                ...
                for key, value in self.get_current_visuals().items():
                    # print(value.shape)
                    self.writer.add_images(key, value[:,:3,:,:])
            if self.ema_scheduler is not None:
    ...
    def val_step(self):
        ...
        with torch.no_grad():
            for val_data in tqdm.tqdm(self.val_loader):
                ...
                if self.opt['distributed']:
                    if self.task in ['inpainting','uncropping']:
                        self.output, self.visuals = self.netG.module.restoration(self.cond_image, self.cond_image,
                            y_0=self.gt_image, mask=self.mask, sample_num=self.sample_num)
                    else:
                        self.output, self.visuals = self.netG.module.restoration(self.cond_image, sample_num=self.sample_num)
                else:
                    if self.task in ['inpainting','uncropping']:
                        self.output, self.visuals = self.netG.restoration(self.cond_image, y_0=self.gt_image, mask=self.mask, sample_num=self.sample_num)
                    else:
